...
**This skill ships no address table, on purpose.** Both sources below now work, re-verified on
2026-08-30 following the full-stack contract redeploy completed 2026-08-24:

- **Hop 1 now works.** The published deploy guide's `#deployment-addresses` table was empty
  ("Coming soon" in every cell) as of 2026-08-11; it now lists real addresses for
  `PermissionsAdapterFactory`, `PermissionedPositionManager`, `PermissionedHooks`, the Universal
  Router, `V4Quoter`, and `MixedRouteQuoterV2` on both mainnet and Sepolia, and they match hop 2
  exactly.
...
redeployed again in the full-stack redeploy completed 2026-08-24, changing these
